...
class LDDTCalculatorWeighted:
    """
    Local Distance Difference Test (lDDT) calculator using Cα coordinates with distance weighting.
    ...
    as described in the original paper, with distance weighting for better
    sensitivity to local structural changes.
    """
    
    def __init__(self, distance_thresholds: List[float] = None, 
                 max_distance: float = 15.0, weight_exponent: float = 0.0):
        """
        Initialize lDDT calculator with distance weighting.
        ...
        Args:
            ...
            weight_exponent: Exponent for distance weighting (1.0 = linear, 2.0 = squared, etc.)
        """
    ...
    def _calculate_distance_matrix(self, coords: np.ndarray) -> np.ndarray:
        """
        Calculate pairwise distance matrix for Cα atoms using vectorized computation.
    ...
    def _calculate_residue_lddt(self, residue_idx: int, 
        ...
        """
        Calculate lDDT score for a single residue with distance weighting.
        ...
        # Calculate distance differences and weights
        distance_diffs = []
        weights = []
        for neighbor in neighbors:
            ...
            if ref_dist > 0:  # Valid distance in reference
                ...
                # Weight closer distances higher using inverse distance weighting
                # Closer neighbors get higher weights - using configurable exponent
                weight = 1.0 / ((ref_dist + 1e-6) ** self.weight_exponent)
        ...
        # Calculate weighted scores for each threshold
        threshold_scores = []
        for threshold in self.distance_thresholds:
            # Weighted count of distances within threshold
            within_threshold_weighted = sum(weights[i] for i, diff in enumerate(distance_diffs) if diff <= threshold)
            score = within_threshold_weighted  # Already normalized since weights sum to 1
...
def example_usage():
    """
    Example usage of the weighted lDDT calculator assuming one-to-one correspondence.
    """
    np.random.seed(42)
    N = 100
    ...
    calculator = LDDTCalculatorWeighted()
