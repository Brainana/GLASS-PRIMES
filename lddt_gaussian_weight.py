...
class LDDTCalculatorGaussian:
    ...
    def __init__(self, distance_thresholds: List[float] = None, 
        ...
        Args:
            ...
            sigma: Standard deviation for Gaussian weighting (in Å)
        """
    ...
    def _calculate_residue_lddt(self, residue_idx: int, 
        ...
        """
        Calculate lDDT score for a single residue, with Gaussian spatial proximity weighting.
        Args:
        ...
        for neighbor in neighbors:
            ...
            if model_dist > 0:  # Valid distance in model
                ...
                # Gaussian weighting
                w = np.exp(- (ref_dist ** 2) / (2 * self.sigma ** 2))
                weights.append(w)
        ...
        for threshold in self.distance_thresholds:
            within_threshold = np.array([diff <= threshold for diff in distance_diffs], dtype=float)
            # Weighted average
            score = np.sum(within_threshold * weights)
