...
class TMLDDTLoss(torch.nn.Module):
    ...
    def __init__(self, alpha=0.7, beta=0.3, gamma=0.1):
        ...
        self.gamma = gamma  # Weight for weighted loss
    ...
    def forward(self, new_emb1, new_emb2, global_emb1, global_emb2, 
        ...
        for i in range(batch_size):
            ...
            if tm_score >= 0.4:
                ...
                residue_loss, weighted_loss = self._compute_residue_loss_with_lddt(
                    new_emb1[i], new_emb2[i], lddt_scores[i], 
                    seqxA_list[i], seqM_list[i], seqyA_list[i]
                )
                pair_loss = self.alpha * residue_loss + self.beta * global_loss + self.gamma * weighted_loss
        ...
        if valid_lddt_pairs == 0:
            return avg_global_loss, {
                ...
                'weighted_loss': torch.tensor(0.0, device=new_emb1.device)
            }
        
        avg_residue_loss = total_residue_loss / valid_lddt_pairs
        avg_weighted_loss = total_weighted_loss / valid_lddt_pairs
    ...
    def _compute_residue_loss_with_lddt(self, emb1, emb2, lddt_scores, seqxA, seqM, seqyA):
        ...
        if not alignment:
            return torch.tensor(0.0, device=emb1.device), torch.tensor(0.0, device=emb1.device)
        ...
        aligned_targets = lddt_scores.to(emb1.device, torch.float32)[list(model_indices)]
        ...
        mean_lddt = torch.mean(aligned_targets)
        weights = torch.abs(aligned_targets - mean_lddt)
        weights = weights / (weights.sum() + 1e-8)  # normalize
        
        per_residue_loss = torch.abs(similarities - aligned_targets)
        weighted_loss = torch.sum(weights * per_residue_loss)
        
        return residue_loss, weighted_loss